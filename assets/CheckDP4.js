function CheckDP4() {
    ...
    const dataRange = worksheet.getRange("B66");
    let data = dataRange.getValue();
  
    // Name data 
    let sodaConcentration = data;
    let responsible = worksheet.getRange("D66").getValue();
    
    // Get measurement timestamp and format it
    let rawTimestamp = worksheet.getRange("E66").getValue();
    ...
    let notified = worksheet.getRange("F66");
    let sendNotification;
  
    // Get notification time value
    let rawNotificationTime = worksheet.getRange("G66").getValue();
    ...
    let setNotificationTime = (time) => {worksheet.getRange("G66").setValue(time)}
