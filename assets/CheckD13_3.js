function CheckD13_3() {
  ...
  const dataRange = worksheet.getRange("B28:B33");
  ...
  let responsible = worksheet.getRange("D28").getValue();
  
  // Get measurement timestamp and format it
  let rawTimestamp = worksheet.getRange("E28").getValue();
  ...
  let notified = worksheet.getRange("F28");
  let sendNotification;

  // Get notification time value
  let rawNotificationTime = worksheet.getRange("G28").getValue();
  ...
  let setNotificationTime = (time) => {worksheet.getRange("G28").setValue(time)}
