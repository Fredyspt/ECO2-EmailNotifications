function CheckDP2() {
  ...
  const dataRange = worksheet.getRange("B39");
  let data = dataRange.getValue();

  // Name data 
  let sodaConcentration = data;
  let responsible = worksheet.getRange("D39").getValue();
  
  // Get measurement timestamp and format it
  let rawTimestamp = worksheet.getRange("E39").getValue();
  ...
  let notified = worksheet.getRange("F39");
  let sendNotification;

  // Sets notificaton time on cell  
  let setNotificationTime = (time) => {worksheet.getRange("G39").setValue(time)}
  // Get notification time value
  let rawNotificationTime = worksheet.getRange("G39").getValue();
