function CheckD13_2() {
  ...
  const dataRange = worksheet.getRange("B20:B25");
  let data = dataRange.getValues();

  // Name data 
  let viscosityNoe = data[0];
  let solidsNoe = data[1];
  let thicknessNoe = data[2];
  let viscosityNoria = data[3];
  let solidsNoria = data[4];
  let thicknessNoria = data[5];
  let responsible = worksheet.getRange("D20").getValue();
  
  // Get measurement timestamp and format it
  let rawTimestamp = worksheet.getRange("E20").getValue();
  ...
  let chemicals = [vNoeOk, sNoeOk, tNoeOk, vNoriaOk, sNoriaOk, tNoriaOk];
  ...
  let notified = worksheet.getRange("F20");
  let sendNotification;

  // Sets notificaton time on cell  
  var setNotificationTime = (time) => {worksheet.getRange("G20").setValue(time)}
  // Get notification time value
  let rawNotificationTime = worksheet.getRange("G20").getValue();
